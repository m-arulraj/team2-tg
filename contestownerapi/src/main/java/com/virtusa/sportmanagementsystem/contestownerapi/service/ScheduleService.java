...
public class ScheduleService {
	...
	/*
	 * public boolean dateComparison(String startDate, String completionDate, String
	 * matchDate) throws ParseException {
	 * logger.debug("Comparing MatchDate with Starting and Completion Dates"); Date
	 * javaStartDate = new SimpleDateFormat("yyyy-MM-dd").parse(startDate); Date
	 * javaCompletionDate = new
	 * SimpleDateFormat("yyyy-MM-dd").parse(completionDate); Date javaMatchDate =
	 * new SimpleDateFormat("yyyy-MM-dd").parse(matchDate); if
	 * (javaStartDate.compareTo(javaMatchDate) <= 0) {
	 * logger.debug("Starting Date is Same/Before Match Date"); if
	 * (javaCompletionDate.compareTo(javaMatchDate) >= 0) {
	...
	public Schedule createSchedule(Schedule schedule) throws ParseException {
		...
		/* if (contest != null) { */
			/*
			 * startDate = contest.getStartingDate(); completionDate =
			 * contest.getCompletionDate(); if (dateComparison(startDate, completionDate,
			 * matchDate)) { return null; } else {
			 */
